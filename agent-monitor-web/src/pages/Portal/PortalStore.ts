 ...
 *
 * 500 → 1000：实测真实会话 `9168ec90`（924 条消息 / 28 个子代理），留 500 条时链上
 * 只有 7 次派活记录还在窗口里、留 1000 条有 26 次 —— **多数智能体卡因此能就地插在
 * 它真正那一步上**，而不是按时间落位。代价约 +134 KB/会话。
 *
 * **这是体验，不是正确性。** 卡片存不存在与这个数**无关**：子代理由子任务清单
 * （`/subtasks`，全量、不受窗口限制）决定，消息流只决定它插在哪一步
 * （见 `TerminalFeed` 的 `attachLooseAgents`）。把这个数调回 500 甚至 80，
 * 28 张卡仍然是 28 张 —— 实测过，只是更多卡片改为按时间落位。
 */
const MAX_MESSAGES_PER_TASK = 1000;
...
class PortalStore {
   ...
   *
   * 为什么 404 还要再判一次设备在不在线：机器掉线超过离线门槛（hub
   * `am_core::heartbeat::OFFLINE_AFTER_SECS`），hub 会把**它名下的任务一起丢掉**，
   * 于是「设备离线」会以 `404 任务不存在` 的形式出现（门槛还是 10s 时实测：离线 4s
   * 是 500，8s 起变 404）。只看 code 的话，用户会被告知
