...
pub struct MachineEntry {
    ...
    /// **临时诊断**：上次打印过的「等待选择」判定摘要（会话 ID → 摘要）。
    ///
    /// 只为查「终端明明在等选择，钉钉却没推」——本地拿真实会话跑过客户端解析器，role 序列
    /// 正常、判定本该触发，差的是 hub 侧那一轮的运行时状态。摘要不变就不重复打（上报 1.5s
    /// 一轮，不限流会刷屏）。定位到之后连同这个字段一起删掉。
    pub select_diag: HashMap<String, String>,
